...
(async()=>{
 ...
 let latest=null;
 const data=()=>({user,settings:{},home:[],materials:[{id:1,title:'Закрытый материал',locked:true,is_free:false,tags:[]}],categories:[],tags:[],courses:[],consultation:{}});
 ...
 await page.locator('.material-card.locked').click();
 await page.getByRole('dialog').waitFor();
 await page.getByRole('button',{name:'Пока не сейчас',exact:true}).click();
 await page.locator('.material-card.locked').focus();
 ...
 console.log('PASS: locked material click and keyboard, modal dismissal, expanded payment, personal tariff, 320/375/430 layout, receipt, approval, banner hidden for member');
