...
class LearningAdmin:
    ...
    def render(self, categories, materials, courses, lessons, home, consultation, settings, notice):
        ...
        </main><script>(function(){{const main=document.querySelector('main'),nav=document.querySelector('.tabs'),hs=[...main.querySelectorAll('h2')],map={{'Настройки Mini App':'Настройки','Категории':'Материалы','Материалы':'Материалы','Курсы':'Курсы','Уроки курса':'Уроки','Главная':'Главная','Консультации':'Консультации'}};let cp,mp;hs.forEach((h,i)=>{{const p=document.createElement('div');p.className='tab-panel';h.parentNode.insertBefore(p,h);let n=h;while(n&&n!==hs[i+1]){{const next=n.nextSibling;p.appendChild(n);n=next}}if(h.textContent.trim()==='Категории')cp=p;if(h.textContent.trim()==='Материалы')mp=p;const b=document.createElement('button');b.type='button';b.className='tab';b.dataset.heading=h.textContent.trim();b.textContent=map[h.textContent.trim()]||h.textContent.trim();b.onclick=()=>{{document.querySelectorAll('.tab-panel').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));p.classList.add('active');b.classList.add('active');history.replaceState(null,'','#'+i)}};nav.appendChild(b)}});if(cp&&mp){{[...cp.childNodes].forEach(n=>mp.appendChild(n));const b=[...nav.children].find(x=>x.dataset.heading==='Категории');if(b)b.remove();cp.remove()}}const b=nav.querySelector('[data-heading="Материалы"]');if(b)b.click()}})();</script></body></html>'''
