...
class CourseAdmin:
    ...
    async def course_snapshot(self, db, course_id: int) -> dict | None:
        row = await (await db.execute(
            """SELECT title,description,cover_url,sequential_access,completion_title,completion_text,
                      completion_recommendation,next_course_id,is_visible,status,is_free
    ...
    async def analytics_html(self, db, course_id: int, lessons) -> str:
        ...
        video_stats = await self.owner.rows(
            db,
            """SELECT b.id,b.title,l.title lesson_title,
                      COUNT(DISTINCT CASE WHEN e.event_type='start' THEN e.telegram_id END) started_count,
                      COUNT(DISTINCT CASE WHEN e.event_type='complete' THEN e.telegram_id END) completed_count,
                      COUNT(e.id) event_count
               FROM mini_app_course_blocks b
               JOIN mini_app_course_units l ON l.id=b.lesson_id
               LEFT JOIN mini_app_course_video_events e ON e.block_id=b.id
               WHERE l.course_id=? AND b.block_type='video'
               GROUP BY b.id ORDER BY l.sort_order,b.sort_order,b.id""",
            (course_id,),
        )
    ...
    async def course_editor(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            recommendations = await self.recommendations_html(db, int(course['id'])) if course else '<section class="panel"><h2>Связанные материалы</h2><p>Сохраните курс, чтобы добавить рекомендации после прохождения.</p></section>'
            ...
            await db.close()
        c = course or {"is_free": 0, "id": "", "title": "", "description": "", "outcome": "", "duration_label": "", "cover_url": "", "is_visible": 0, "sequential_access": 0, "completion_title": "", "completion_text": "", "completion_recommendation": "", "next_course_id": None}
        ...
        try:
            ...
            blocks = await self.owner.rows(
                ...
                """SELECT b.*,m.title material_title,m.short_description material_description,
                          j.status video_status,j.error video_error,j.poster_name video_poster_name
                   FROM mini_app_course_blocks b
    ...
    def block_card(self, block, lesson_id: int, course_id: int) -> str:
        ...
        if block_type == "video" and block["content"]:
            ...
            preview = f'<div class="block-preview">{status}<video src="{esc(block["content"])}"{poster}{controls} preload="metadata"></video>{retry}</div>'
    ...
    async def recommendations_html(self, db, course_id):
        linked = await self.owner.rows(db, 'SELECT m.id,m.title FROM mini_app_material_course_relations r JOIN mini_app_materials m ON m.id=r.material_id WHERE r.course_id=? ORDER BY m.title', (course_id,))
        selected = {row['id'] for row in linked}
        available = await self.owner.rows(db, "SELECT id,title FROM mini_app_materials WHERE status='published' AND library_visible=1 ORDER BY title")
        cards = ''.join(f'''<div class="row" style="margin:12px 0;gap:12px"><b>{esc(row['title'])}</b><form method="post" action="{self.action_url}" onsubmit="return confirm('Убрать связь с материалом в обе стороны?')"><input type="hidden" name="action" value="course_related_material_delete"><input type="hidden" name="course_id" value="{course_id}"><input type="hidden" name="material_id" value="{row['id']}"><button class="danger" aria-label="Убрать рекомендацию">×</button></form></div>''' for row in linked)
        options = ''.join(f'<option value="{row["id"]}">{esc(row["title"])}</option>' for row in available if row['id'] not in selected)
        form = f'''<form method="post" action="{self.action_url}"><input type="hidden" name="action" value="course_related_material_add"><input type="hidden" name="course_id" value="{course_id}"><label class="field">Добавить материал<select name="material_id" required><option value="">Выберите материал…</option>{options}</select></label><button>＋ Добавить</button></form>''' if options else '<p class="hint">Нет других опубликованных материалов для добавления.</p>'
        return f'<section class="panel"><h2>Связанные материалы</h2><p class="hint">Показываются после прохождения курса. Материалы, ссылающиеся на этот курс, уже добавлены. Связь двусторонняя: добавление и удаление отражаются и в материале.</p>{cards or "<p>Рекомендаций пока нет. Пустой блок участник не увидит.</p>"}{form}</section>'
        ...
        try:
            if action in {'course_move', 'course_reorder'}:
                ...
                if action.endswith('_add'):
                    target = await (await db.execute("SELECT id FROM mini_app_materials WHERE id=? AND status='published' AND library_visible=1", (related_id,))).fetchone()
                    if not target:
                        raise web.HTTPBadRequest(text='Выберите опубликованный материал')
                    await db.execute('INSERT OR IGNORE INTO mini_app_material_course_relations(material_id,course_id) VALUES(?,?)', (related_id,course_id))
                else:
                    await db.execute('DELETE FROM mini_app_material_course_relations WHERE material_id=? AND course_id=?', (related_id,course_id))
            elif action == "course_inline_upload":
                ...
                if revision["entity_type"] == "course":
                    ...
                    await db.execute('UPDATE mini_app_courses SET is_free=? WHERE id=?', (1 if snapshot.get('is_free') else 0, course_id))
                ...
                await db.execute(
                    """UPDATE mini_app_course_units SET
                           title=?,description=?,module_id=?,is_required=?,previous_button_label=?,
                           next_button_label=?,finish_button_label=?,updated_at=?
                       WHERE id=? AND course_id=?""",
                    (
                        str(form.get("title") or "").strip(),
                        str(form.get("description") or "").strip() or None,
                        module_id,
                        1 if form.get("is_required") == "1" else 0,
                        str(form.get("previous_button_label") or "").strip() or None,
                        str(form.get("next_button_label") or "").strip() or None,
                        str(form.get("finish_button_label") or "").strip() or None,
                        now,
                        lesson_id,
                        course_id,
                    ),
            ...
            elif action == "course_video_retry":
                block_id = int(form.get("block_id") or 0)
                row = await (await db.execute(
                    "SELECT stored_name FROM mini_app_course_blocks WHERE id=? AND lesson_id=? AND block_type='video'",
                    (block_id, lesson_id),
                )).fetchone()
                if not row or not row["stored_name"]:
                    raise web.HTTPNotFound(text="Видео не найдено")
                await db.execute(
                    """UPDATE mini_app_video_jobs SET status='queued',error=NULL,started_at=NULL,
                    ...
                    settings = json.dumps(
                        {
                            ...
                            "required": bool(form.get("test_required")),
                        },
                        ensure_ascii=False,
                    )
                elif block_type == "assignment":
