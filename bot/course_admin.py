...
class CourseAdmin:
    ...
    def block_card(self, block, lesson_id: int, course_id: int) -> str:
        ...
        fields = f'<label class="field">Заголовок<input name="title" value="{esc(block["title"] or "")}" placeholder="Необязательно"></label>'
        if block_type == "longread":
            fields += f'<label class="field">Текст<textarea name="content" style="min-height:220px" placeholder="Вставьте или напишите текст урока">{esc(block["content"] or "")}</textarea></label><div class="format-hint">На следующем шаге подключим сюда тот же визуальный редактор, что и в материалах.</div>'
            ...
            fields += f'{preview}<label class="field">Описание<textarea name="description">{esc(block["description"] or "")}</textarea></label><label class="field">Заменить файл<input type="file" name="block_file" accept="{"video/mp4" if block_type == "video" else "image/*"}"></label>'
        return f'''<article class="block-card"><span class="block-type">{esc(self.block_labels.get(block_type, block_type))}</span><form method="post" action="{self.action_url}" enctype="multipart/form-data"><input type="hidden" name="action" value="course_block_save"><input type="hidden" name="course_id" value="{course_id}"><input type="hidden" name="lesson_id" value="{lesson_id}"><input type="hidden" name="block_id" value="{block["id"]}">{fields}<div class="block-actions"><button>Сохранить блок</button></div></form><form method="post" action="{self.action_url}" style="position:absolute;right:16px;top:16px" onsubmit="return confirm('Удалить блок?')"><input type="hidden" name="action" value="course_block_delete"><input type="hidden" name="course_id" value="{course_id}"><input type="hidden" name="lesson_id" value="{lesson_id}"><input type="hidden" name="block_id" value="{block["id"]}"><button class="ghost">×</button></form></article>'''

    def add_block_form(self, lesson_id: int, course_id: int) -> str:
        return f'''<details class="add-block"><summary>＋ Добавить блок</summary><form method="post" action="{self.action_url}" enctype="multipart/form-data"><input type="hidden" name="action" value="course_block_add"><input type="hidden" name="course_id" value="{course_id}"><input type="hidden" name="lesson_id" value="{lesson_id}"><label class="field">Тип блока<select name="block_type"><option value="longread">Лонгрид</option><option value="video">Видео</option><option value="image">Изображение</option><option value="test">Тест</option></select></label><label class="field">Заголовок<input name="title" placeholder="Необязательно"></label><label class="field">Текст или вопрос<textarea name="content" placeholder="Для видео и изображения можно оставить пустым"></textarea></label><label class="field">Файл<input type="file" name="block_file" accept="image/*,video/mp4"></label><p class="hint">Для теста сначала создайте блок, затем добавьте варианты ответов.</p><button>Добавить блок</button></form></details>'''
    ...
    async def action(self, request: web.Request, form) -> web.Response:
        ...
        try:
            if action == "course_save":
                ...
                if course_id:
                    rows = await self.owner.rows(db, "SELECT stored_name FROM mini_app_course_blocks b JOIN mini_app_course_units l ON l.id=b.lesson_id WHERE l.course_id=? AND stored_name IS NOT NULL", (course_id,))
                ...
                if not course_id or not title:
                    raise web.HTTPBadRequest(text="Введите название модуля")
                ...
                title = str(form.get("title") or "").strip()
                if not course_id or not title:
                    raise web.HTTPBadRequest(text="Введите название урока")
                ...
                block_id = int(form.get("block_id") or 0)
                row = await (await db.execute("SELECT stored_name FROM mini_app_course_blocks WHERE id=? AND lesson_id=?", (block_id, lesson_id))).fetchone()
