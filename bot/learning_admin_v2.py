...
from bot.media_embed import youtube_embed_url, youtube_video_id
...
class RichTextSanitizer(HTMLParser):
    allowed_tags = {"pre", "code", "div", "p", "br", "strong", "b", "em", "i", "u", "s", "h2", "h3", "ul", "ol", "li", "blockquote", "a", "img", "video", "source", "figure", "figcaption", "hr", "iframe"}
    void_tags = {"br", "img", "source", "hr", "iframe"}
    allowed_attributes = {
        "a": {"href", "title"},
        "img": {"src", "alt", "title"},
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        tag = tag.lower()
        if tag in {"script", "style", "object", "form"}:
            self.blocked_depth += 1
            return
        if self.blocked_depth:
            return
        if tag == "iframe":
            video_id = youtube_video_id(dict(attrs).get("src"))
            if video_id:
                self.output.append(
                    f'<iframe src="{youtube_embed_url(video_id)}" title="Видео YouTube" '
...
class LearningAdmin:
    ...
    async def article_editor(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            related_rows = await self.rows(
                db,
                """SELECT r.related_material_id AS id,m.title,m.short_description,m.cover_url
                   FROM mini_app_material_relations r JOIN mini_app_materials m ON m.id=r.related_material_id
                   WHERE r.material_id=? ORDER BY r.sort_order,r.related_material_id""",
                (material["id"],),
            ) if material else []
            ...
            available_related = await self.rows(
                ...
                """SELECT id,title,short_description FROM mini_app_materials
                   WHERE id<>? AND status='published' AND library_visible=1
                   ORDER BY updated_at DESC,title""",
                (material["id"] if material else 0,),
            )
        ...
        course_context = (
            '<div class="course-context"><span>Материал урока</span><div><b>Этот лонгрид является частью курса</b>'
            '<p>Он открывается только внутри урока и не публикуется как отдельный материал библиотеки.</p></div></div>'
            if is_course_material else ""
        )
        free_access = "" if is_course_material else f'''<label style="display:flex;align-items:flex-start;gap:10px;margin-top:18px;padding:14px;border:1px solid var(--line);border-radius:12px;cursor:pointer"><input type="checkbox" name="is_free" value="1" {"checked" if m["is_free"] else ""} style="width:18px;height:18px;margin:2px 0 0;flex:none"><span><b>Доступен бесплатно</b><br><span class="hint">Материал смогут открыть пользователи без оплаченной подписки.</span></span></label>'''
        details_heading = "О лонгриде" if is_course_material else "Материал"
        details_hint = "Название и короткое описание этой части урока." if is_course_material else "Название и короткий анонс для карточки в приложении."
        short_description_placeholder = "О чём этот лонгрид — 1–3 предложения" if is_course_material else "О чём этот материал — 1–3 предложения"
        ...
        ) or '<p class="empty-note">Связанных материалов пока нет. Пользователь этот блок не увидит.</p>'
        related_options = "".join(
            f'<option value="{row["id"]}">{esc(row["title"])}</option>'
            for row in available_related if int(row["id"]) not in related_ids
        )
        related_add_form = (
        ...
        return web.Response(text=f'''<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title} — Nastaunik</title><script defer src="/mini-app/static/admin_uploads.js?v=3"></script><script defer src="/mini-app/static/admin_autosave.js?v=1"></script><script defer src="/mini-app/static/admin_material.js?v=11"></script><style>
        ...
        try:
            if action == "video_status":
                ...
                values = (
                    ...
                    str(form.get("full_description") or "").strip() or None,
                    0 if is_course_material else (1 if form.get("is_free") == "1" else 0),
                    now,
                )
                if not values[0]:
                ...
                if item_id:
                    ...
                    if is_course_material:
                        await db.execute("UPDATE mini_app_materials SET library_visible=0 WHERE id=?", (item_id,))
                else:
                ...
                if not editor_id or not related_id or editor_id == related_id:
                    raise web.HTTPBadRequest(text="Материал не указан")
                if action == "related_material_add":
                    target = await (await db.execute(
                        "SELECT id FROM mini_app_materials WHERE id=? AND status='published' AND library_visible=1",
                        (related_id,),
                    )).fetchone()
                    if not target:
                        raise web.HTTPBadRequest(text="Можно связать только опубликованный материал")
                    cur = await db.execute(
                        "SELECT COALESCE(MAX(sort_order),-1)+1 FROM mini_app_material_relations WHERE material_id=?",
                        (editor_id,),
                    )
                    order = (await cur.fetchone())[0]
                    await db.execute(
                        "INSERT OR IGNORE INTO mini_app_material_relations(material_id,related_material_id,sort_order,created_at) VALUES(?,?,?,?)",
                        (editor_id, related_id, order, now),
                    )
                else:
                    await db.execute(
                        "DELETE FROM mini_app_material_relations WHERE material_id=? AND related_material_id=?",
                        (editor_id, related_id),
                    )
